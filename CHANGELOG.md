...
## v0.6 存档候选 - 2026-09-11（待推送、CI与打标签）
...
- **版本与标签边界**：`versionCode 6→7`、`versionName 0.5.0→0.6.0`。本轮预检当场核对本地与远程标签均只到 `v0.4`，不存在 `v0.5` Git标签；因此后续 `v0.6` 实际还会覆盖 `96a3730` 的本机TOTP卡片与 `9bb1254` 的0.5.0版本更新，不能把APK内部版本号0.5.0误写成已有Git标签。推送、CI与附注标签由指挥师后续执行，本条不提前宣称远程已存档。
...
- **未验证**：未在真机操作显示/隐藏按钮、截图、无障碍读屏或跨30秒窗口观察；这些不能由JVM测试代报。首次未指定SDK的测试命令未进入编译，沙箱内读取SDK又被权限阻断；改为仅在命令环境中指向既有SDK并获准读取后，完整测试、Lint和构建均通过，未创建或提交 `local.properties`。
- **逐文件改动（本批新增）**：
