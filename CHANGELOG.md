...
- **验证存档方独立核到的**：零网络/认证/安全文件改动；三个 Activity 的控制流关键字分布与 v0.6 完全相同；96 行中文字面量 ↔ 96 行 `R.string`，`strings.xml` 新增 115 条；无测试删除；无主机名（例行核对中 3 处「密钥形态」命中为 diff 头长路径误报，已逐条查看）。gradle test 未由验证方重跑：用户已在 Vivo V2405A 覆盖安装 QA 包并使用，实施方实跑 prod/qa 各 42/42。
- **未验证**：配对页真机截图缺（QA 已绑定、配对 Activity 不导出）；375/1280 设备布局、TalkBack、真实登录与生物识别未由验证方实测。

## 2026-09-12 次要按钮与真机复核

- “重试”“开始配对”复用既有 SecondaryButton；禁用态背景引用 divider 令牌。保留原系统按钮形状，主按钮和危险按钮未变。用户查看修正版后确认保留。
- Vivo V2405A 真机安装 QA 包，保存主界面与设置页截图；原生屏幕 1260×2800、560 dpi，折合 360dp 宽。实际覆盖启用的“重试”与禁用的“隐藏验证码”，未触发真实生物识别。
- 最终测试 XML：prod/qa 各 42 项、0 失败、0 错误；QA lint XML 0 errors / 7 warnings，产物更新时间 23:07。QA APK 构建、安装成功，git diff --check 通过。
- 配对页仍未取得真机证据：QA 包已有加密绑定，Android 拒绝直接打开非导出 Activity。保留既有绑定，未清数据或重置配对。375dp / 1280dp、字体缩放及读屏仍未实测；下面初次记录中的“无设备”是当时状态，本条补充随后接入真机的事实。

## 2026-09-12 延展首页暗色档案风格到随身管理界面

- 三个 Activity 的 XML 统一暗色、mono 编号和层级留白；设置页分离连接与维护操作，配对字段增加常驻标签；登录、站点近况、备份、动态码保留真实状态与原可见性控制。主标题 36sp、正文 16sp、说明 12sp，间距及控件尺寸统一引用新增 dimens 令牌，危险操作与普通按钮分色。
- 三个 Activity 的 97 处硬编码显示字符串迁入 strings.xml，插值仍取原字段；逐文件重建对比证明 Kotlin 差异仅资源读取与 R 导入。布局原 32 + 8 + 8 个 ID 全部保留。未改网络请求、配对、会话、生物识别互斥、TOTP 显隐及安全存储逻辑，未更改版本号。
- 实测：prod / qa JVM 测试各 **42 项、0 失败、0 错误**；QA Debug 编译成功，QA Lint **0 errors / 7 warnings**。首次 Lint 暴露 API 27 属性与 7 处整数格式资源错误，已在实现侧修正，未放宽检查。`git diff --check` 通过。
- **未验证，不得代报通过**：本机 adb 无已连接设备，emulator 无已配置 AVD，故未提供三界面的真机 / 模拟器截图，未实测 375 / 1280 设备布局或生物识别。UI 纯状态 / 网络诊断辅助类中既有中文仍保留：部分字符串参与错误匹配或独立 JVM 契约，未为抽取文案改其行为。未安装正式 App、未连接服务器、未推送。
